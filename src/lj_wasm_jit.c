...
static int wasm_emit_arith(WasmTraceCtx *ctx, IRRef ref, IRIns *ir)
{
  ...
  if (type == LJ_WASM_TYPE_F64) {
    ...
    op = ir->o == IR_ADD ? LJ_WASM_OP_I32_ADD :
	 ir->o == IR_SUB ? LJ_WASM_OP_I32_SUB :
	 ir->o == IR_MUL ? LJ_WASM_OP_I32_MUL : LJ_WASM_OP_I32_REM_S;
  }
...
static uint32_t wasm_guard_exitno(WasmTraceCtx *ctx, IRRef ref)
{
  const GCtrace *T = ctx->T;
  uint32_t exitno = 0;
  if (T->snap != NULL && T->nsnap != 0) {
    SnapNo i;
    for (i = 0; i < T->nsnap; i++) {
      if (T->snap[i].ref > ref)
	break;
...
static int wasm_emit_le_guard(WasmTraceCtx *ctx, IRRef ref, IRIns *ir)
{
...
static int wasm_emit_eq_guard(WasmTraceCtx *ctx, IRRef ref, IRIns *ir)
{
  uint8_t type = wasm_ref_fixed_type(ctx->T, ir->op1);
  if (!type) type = wasm_ref_fixed_type(ctx->T, ir->op2);
  if (!type) type = wasm_ir_valtype(ir);
...
static int wasm_emit_trace_body(lua_State *L, const GCtrace *T, SBuf *body)
{
  ...
  for (ref = REF_FIRST; ref < T->nins; ref++) {
	...
	return 0;
      break;
    case IR_ABC:
      if (!wasm_emit_abc_guard(&ctx, ref, ir))
	return 0;
      break;
    case IR_AREF:
	...
	return 0;
      break;
    case IR_LE:
      if (!wasm_emit_le_guard(&ctx, ref, ir))
	return 0;
      break;
    case IR_EQ:
      if (!wasm_emit_eq_guard(&ctx, ref, ir))
	return 0;
      break;
    case IR_PHI:
      break;
